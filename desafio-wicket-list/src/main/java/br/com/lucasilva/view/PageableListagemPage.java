...
public class PageableListagemPage extends TemplatePage {
	private static final long serialVersionUID = 4291324880250962408L;

	List<User> regPesList = new ArrayList<User>();
	...
	private PageableListView<User> regPesView;
	
	public PageableListagemPage() {
		cadModal = new ModalWindow("cadModal");
		add(Utils.addLink("homeLink", HomePage.class), Utils.addLink("cadLink", PageableListagemPage.class));

		addListView("listView");

		final AjaxPagingNavigator nav = new AjaxPagingNavigator("paginacao", regPesView);
	...
	private void addListView(String id) {
		...
		regPesView = new PageableListView<User>(id, loader, 15) {
			...
			protected void populateItem(ListItem<User> item) {
				...
				item.add(new AjaxLink<User>("edit") {
					...
					private List<User> tempRegPes = new ArrayList<User>();

					@Override
					public void onClick(AjaxRequestTarget target) {

						tempRegPes = regPesList;
						...
						FormPanel formPanel = new FormPanel(cadModal.getContentId(), regPes, cadModal) {
							...
							public void executarAoSalvar(AjaxRequestTarget target, User pessoa) {
								tempRegPes.add(itemAux.getIndex(), pessoa);
								regPesList.remove(itemAux.getIndex());
								regPesList = tempRegPes;
				...
				item.add(new AjaxLink<User>("remove") {
					...
					@Override
					public void onClick(AjaxRequestTarget target) {
						ExclusaoPanel excPanel = new ExclusaoPanel(excluirMod.getContentId(), regPesList, itemAux,
								regPes) {
